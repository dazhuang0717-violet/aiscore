...
- 成本考量：创意公益项目、低成本社交媒体运营、Pitch 媒体（主动沟通获得的报道）由于投入成本极低且转化路径直接，在获客效能上具有绝对优势，应大胆给予高分（8-10 分）。
...
- 【特别注意】传统大型展会（如进博会 CIIE）虽然声量巨大，但由于其受众泛化、获客路径极长且单客获取成本极高，其“获客效能”得分必须保持在较低水平（通常 2-4 分），除非项目描述中明确展示了极其高效的数字转化闭环。
...
- 旗舰级创意公益项目（如：Run for Her）：这是标杆项目，[真需求]、[声量]、[获客效能] 全都应给高分（9-10分）。其声量应能与进博会抗衡，且获客效能远超进博会。
- 创意/公益宣教类项目（如：从容生活）：[真需求] 和 [获客效能] 应给极高分（9-10分），但 [声量] 相对较低（4-6分，通常低于进博会）。
- 传统展会/品牌展示类（如：进博会 CIIE）：[声量] 和 [媒体分级] 给高分（8-10分），但 [获客效能] 必须给低分（2-4分），[真需求] 视匹配度而定（通常 5-7 分）。总分应明显低于上述两类创意项目。
- 政策/医保落地项目：[声量] 和 [媒体分级] 应给高分（8-10分），因为这类项目通常伴随官方解读和极广泛的社会传播；但 [获客效能] 通常处于中低水平。
- 进博会 (CIIE) 内部差异化评分：
    - 肺癌 (LC) 和 肝癌 (GIGU)：由于核心信息极其精准且单一（如“无瘤生存”、“从容未来”），其 [真需求]（核心信息匹配）分数应给高分（8-9分）。
    - 乳腺癌 (BC)：若涉及多个核心信息（如赫双妥和伊那利塞并存），其信息焦点可能相对分散，[真需求] 评分应比 LC/GIGU 略低（通常 6-7 分），除非内容极其精准地同时覆盖了多个要点。
- 【核心原则】请在评分时极其果断。优秀项（创意/公益）必须给 9-10 分，平庸或高成本低效项（传统展会）必须给低分，严禁大量给出 7 分。
`;
...
- 旗舰级创意公益项目（如：Run for Her）：标杆项目，[真需求]、[声量]、[获客效能] 全都应给高分（9-10分）。
- 创意/公益宣教类项目（如：从容生活）：[真需求] 和 [获客效能] 应给极高分（9-10分），但 [声量] 相对较低（4-6分）。
- 传统展会类（如：进博会 CIIE）：[声量] 高（8-10分），但 [获客效能] 必须给低分（2-4分）。总分应明显低于创意项目。
- 政策/医保落地项目：[声量] 和 [媒体分级] 应给高分（8-10分）；但 [获客效能] 通常中低。
- 进博会 (CIIE) 内部差异化：肺癌 (LC) 和 肝癌 (GIGU) 的 [真需求] 分数应高于乳腺癌 (BC)，因为前者信息更聚焦，后者信息点较多导致精准度稀释。
- 【重要】请在评分时极其果断。优秀项必须给 9-10 分，平庸项给 5-6 分，严禁大量给出 7 分。
