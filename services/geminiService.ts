...
- 纯媒体购买/政策落地类（如：高罗华医保落地、进博会 CIIE）：[声量] 和 [媒体分级] 给高分（8-10分），但 [获客效能] 必须严格控制在低分（2-4分）。
    - 【进博会内部差异化】肺癌 (LC) 和 肝癌 (GIGU) 的 [真需求]（核心信息匹配）分数应显著高于乳腺癌 (BC)。因为 LC/GIGU 的信息通常极其聚焦（如“无瘤生存”），而 BC 的信息点往往过于分散。
    - 【获客效能与声量一致性】对于同属于进博会 (CIIE) 的不同癌种项目，其 [获客效能] 应处于相似的低分区间（2-4分），[声量] 也应处于相似的高分区间，不应因为癌种不同而在获客效能上产生巨大鸿沟。
    - 【获客效能梯度】严禁所有项给同一个固定分数。请根据内容的行动号召力，在 2-4 分之间给出有梯度的评分（如 2.5, 3.2, 3.7）。
- 【核心原则】请在评分时极其果断且慷慨。低成本、高影响力的项目必须给 9.5-10 分，高投入、纯买量项目必须在获客效能上给低分，但内部要拉开梯度。
`;
...
- 纯媒体购买/政策落地类（如：高罗华医保落地、进博会 CIIE）：[声量] 高（8-10分），但 [获客效能] 必须给低分（2-4分）。
    - 【进博会内部差异化】肺癌 (LC) 和 肝癌 (GIGU) 的 [真需求] 分数应显著高于乳腺癌 (BC)，因为前者信息更聚焦，后者信息点分散。
    - 【获客效能与声量一致性】对于同属于进博会 (CIIE) 的不同癌种项目，其 [获客效能] 应处于相似的低分区间（2-4分），[声量] 也应处于相似的高分区间，不应因为癌种不同而在获客效能上产生巨大鸿沟。
    - 【获客效能梯度】严禁所有项给同一个固定分数。请根据内容的行动号召力，在 2-4 分之间给出有梯度的评分（如 2.5, 3.2, 3.7）。
- 【重要】请在评分时极其果断且慷慨。低成本高影响力的项目必须给 9.5-10 分，高投入纯买量项目在获客效能上给低分，但内部要拉开梯度。
